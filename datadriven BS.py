# -*- coding: utf-8 -*-
"""
Created on Sun Jan  1 19:12:51 2023
...
w_ic=tf.Variable(0.0)
w_dc=tf.Variable(0.0)
w_fc=tf.Variable(0.0)
w_nb=tf.Variable(0.0)
w_s=tf.Variable(1.0)
...
#add square 
loss =w_s*tf.reduce_sum(tf.square(c_sp-c_s))/len(x_s)
...
plt.plot(x_ic, catp, 'g', label="w_ic", linewidth=4)
plt.plot(s,final_an, label="w_ic", linewidth=4)
plt.plot(s[:],final[0,:],label="w_ic", linewidth=4)




plt.legend(loc="upper left", mode = "expand", ncol = 10)
plt.ylim(0, 90.0)

plt.show()

np.savetxt('datadriven.csv', catp, delimiter=',')
"""
